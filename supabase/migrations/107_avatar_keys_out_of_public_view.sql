-- 107_avatar_keys_out_of_public_view.sql
--
-- Security-Fix: profiles.photos-Einträge haben das Format `path|key|iv`
-- (VOLLSTÄNDIGER AES-256-Schlüssel + IV, 077) und standen über
-- public_profiles sowie get_public_profile(s) für JEDEN authentifizierten
-- Nutzer lesbar. Schlüssel gehören nur autorisierten Betrachtern.
--
-- Ab jetzt liefern View + RPCs nur die PFADE (Legacy-Klartextpfade
-- unverändert); Schlüssel + signierte URL kommen aus der match-media
-- Edge Function (kind=avatar, gleiche Match-/Quiz-Berechtigungsprüfung
-- wie bisher für die URL). Eigene Zeile (profiles-Tabelle direkt) und
-- eigene Uploads bleiben unverändert (volle Refs mit Schlüssel).
--
-- Client-seitig lesen Fremd-Avatare nur noch über match-media-Antwort
-- {url, keyB64, ivB64} (decrypt lokal); Legacy-Einträge ohne Schlüssel
-- kommen als {url} und werden als Klartext angezeigt.

-- public_profiles-View (Basis: 092-Definition, nur photos-Ausdruck ändert
-- sich; Spaltenname/-position bleiben identisch).
...
comment on column public.profiles.photos is
  'Profilbild-Referenzen (Storage-Pfad plus clientseitige AES-Schluesselanteile im Format path|key|iv). 107: View/RPCs liefern nur Pfade; Schluessel nur via match-media an Berechtigte.';

-- get_public_profile (Basis: 092): photos ebenfalls pfad-bereinigt.
...
-- get_public_profiles (Basis: 092): photos ebenfalls pfad-bereinigt.
